...
static void find_last_successor(STree *stree, Vertex *prev_p, Vertex *vertex_p)
{
    ...
    do {
        prev = vertex;
        if(IS_LEAF(vertex)) {
            vertex = LEAF_SIBLING(stree->leaves.first + LEAF_NUM(vertex));
        } else {
            vertex = SIBLING(stree->inner.first + LEAF_NUM(vertex));
        }
...
void walk(STree *stree)
{
    ...
    if(IS_HEADDEPTH_ZERO) {
        ...
        if(IS_LEAF(scanprobe_val)) {
            ...
            Pattern edgepatt = make_patt(text + LEAF_NUM(scanprobe_val) + 1, sentinel - 1);
            Pattern tailpatt = make_patt(stree->tailptr + 1, sentinel - 1);
        ...
        scanprobe = stree->inner.first + LEAF_NUM(scanprobe_val);
    ...
    while(True) {
        ...
        do {
            ...
            if(IS_LEAF(scanprobe_val)) {
                ...
                leafindex = LEAF_NUM(scanprobe_val);
